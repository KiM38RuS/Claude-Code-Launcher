...
ShowFolderSelectionGUI() {
    ...
    myGui := Gui("", "Запуск Claude Code " SCRIPT_VERSION)
    myGui.SetFont("s10")

    ; Текст
    myGui.Add("Text", "x10 y10", "Выберите папку для запуска Claude Code:")
    ...
    ; Кнопка "Обзор"
    browseBtn := myGui.Add("Button", "x+m yp-1 w80 h26", "Обзор...")
    ...
    launchBtn := myGui.Add("Button", "x330 y100 w80 h30 Default", "Запустить")
    launchBtn.OnEvent("Click", (*) => OnLaunchClick(myGui, folderCombo, launchBtn, cancelBtn, statusText))

    cancelBtn := myGui.Add("Button", "x+m y100 w80 h30", "Закрыть")
